...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const authContext = (req as Record<string, unknown>).url?.sb?.[0];
    if (authContext && authContext.auth_user) {
      console.log(
        "[DEBUG] User ID found in JWT context:",
    ...
    try {
      ...
      // Store WhatsApp credentials if it's a Meta connection
      if (requestData.type === "meta") {
        console.log("[DEBUG] Storing WhatsApp credentials for Meta connection");
        const { error: credentialsError } = await adminClient
          .from("whatsapp_credentials")
          .upsert({
            user_id: userId,
            phone_number_id: requestData.phoneNumberId,
        ...
        if (credentialsError) {
          console.error(
            "[DEBUG] Error storing WhatsApp credentials:",
            credentialsError
          );
          throw new Error(
            `Failed to store WhatsApp credentials: ${credentialsError.message}`
          );
        }
      }
