...
const WhatsAppAPIConnect: React.FC<WhatsAppAPIConnectProps> = ({
  onSuccess,
}) => {
  const { toast } = useToast();
  const [isLoading, setIsLoading] = useState(false);
  const [connectionType, setConnectionType] = useState<"meta" | "twilio">(
    "meta"
  );
  ...
  const handleApiConnect = async (
    ...
    e.preventDefault();
    logger.info(`Iniciando conexão com API do WhatsApp - Tipo: ${type}`);
    setIsLoading(true);
    ...
    try {
      ...
      logger.info(`Conectando ${type} API para usuário ${user.id}`);
      ...
      if (response.error) {
        ...
        logger.error(`Erro ao conectar ${type} API: ${response.error}`);
        throw new Error(
          response.error.message || `Failed to connect ${type} API`
        );
      }
      ...
      console.error(
        `Erro ao conectar ${type === "meta" ? "WhatsApp API" : "Twilio"}:`,
        error
      );
      logger.error(
        `Erro ao conectar ${
          type === "meta" ? "WhatsApp API" : "Twilio"
        }: ${error}`
      );
      toast({
        ...
        description:
          errorMessage ||
          `Falha ao conectar com ${
            type === "meta" ? "a API do WhatsApp" : "o Twilio"
  ...
  const handleTwilioConnect = (e: React.FormEvent) =>
    handleApiConnect(e, "twilio");

  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-lg border shadow-sm">
        <h3 className="text-xl font-semibold mb-4">
          Conecte seu WhatsApp Business API
        </h3>

        {!user?.id && (
          <div className="mb-4 p-3 bg-red-50 border border-red-300 rounded-md">
            <p className="text-sm text-red-700">
              <strong>Atenção:</strong> Você precisa estar autenticado para
              conectar a API. Por favor, faça login novamente.
            </p>
          </div>
        )}
        ...
        <RadioGroup
          ...
          onValueChange={(value) =>
            setConnectionType(value as "meta" | "twilio")
          }
          className="mb-6"
        >
          <div className="flex flex-col md:flex-row gap-4">
            <div
              className={`flex items-start space-x-2 p-4 border rounded-lg cursor-pointer ${
                connectionType === "meta" ? "border-green-500 bg-green-50" : ""
              }`}
            >
              ...
              <Label htmlFor="meta" className="cursor-pointer flex-1">
                ...
                <div className="text-sm text-gray-500">
                  Ideal para contas comerciais registradas no Meta Business
                </div>
              </Label>
            </div>
            ...
            >
              ...
              <Label htmlFor="twilio" className="cursor-pointer flex-1">
                ...
                <div className="text-sm text-gray-500">
                  Alternativa para quem não tem uma conta comercial verificada
                </div>
              </Label>
            </div>
          </div>
                  ...
                  <div className="grid gap-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-gray-500">
                        Encontrado na sua conta do Meta Business.
                      </p>
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="meta-phone-id">
                      ...
                      />
                      <p className="text-xs text-gray-500">
                        Disponível no WhatsApp Business Dashboard.
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-xs text-gray-500">
                        Token permanente para acessar a API.
                      </p>
                    </div>
                  </div>
                ...
                <form onSubmit={handleTwilioConnect} className="space-y-4">
                  <div className="grid gap-4">
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-xs text-gray-500">
                        Encontrado no Dashboard do Twilio.
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-xs text-gray-500">
                        Número habilitado para WhatsApp no Twilio.
                      </p>
                    </div>
                  </div>
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <ul className="mt-1 text-xs text-yellow-700 list-disc list-inside space-y-1">
                <li>
                  Seus dados de API são armazenados de forma segura e
                  criptografada.
                </li>
                <li>
                  Você precisa de uma conta verificada no Meta Business para
                  usar a API oficial.
                </li>
                <li>
                  A integração via Twilio é uma alternativa para quem não tem
